...
describe('Microcosm', function() {
  ...
  it ('does not allow promises to trap errors during dispatch', function (done) {
    let app    = new Microcosm()
    let action = () => Promise.resolve(true)
    let error  = new Error('This should be thrown!')
    ...
    assert.throws(app.prepare(action), error.message)

    // Use a timeout here to execute done in the next call stack.
    // This gives the Promise plenty of time to clear.
    setTimeout(() => done(), 100)
  })

  it ('does not allow promises to trap errors on successful app.push callbacks', function (done) {
    let app    = new Microcosm()
    let action = () => Promise.resolve(true)
    let error  = new Error('This should be thrown!')

    assert.throws(function() {
      app.push(action, null, function() {
        throw error
      })
    }, error.message)
  ...
  it ('does not allow promises to trap errors on failed app.push callbacks', function (done) {
    let app    = new Microcosm()
    let action = () => Promise.reject(true)
    let error  = new Error('This should be thrown!')

    assert.throws(function() {
      app.push(action, null, function(err) {
        throw error
      })
    }, error.message)
